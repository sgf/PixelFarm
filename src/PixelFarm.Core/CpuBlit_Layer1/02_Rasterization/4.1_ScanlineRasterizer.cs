...
{
    private readonly VectorClipper _vectorClipper;
    private readonly CellAABlob _cellAABlob;

    private int[] _gammaLut;//current gamma lut
    private bool _useDefaultGammaLut;
    private readonly int[] _orgGammaLut = new int[AA_SCALE]; //original(built-in) gamma table

    private FillingRule _filling_rule;
    private bool _auto_close;

    /// <summary>
    /// multiplied move to start x
    /// </summary>
    private int _mul_start_x;

    /// <summary>
    /// multiplied move to starty
    /// </summary>
    private int _mul_start_y;
    ...
    private Q1Rect _userModeClipBox;
    //---------------
    ...
    //---------------------------------
    //from vector clipper
    private static int upscale(double v) => AggMathRound.iround(v * poly_subpix.SCALE);

    private static int upscale(int v) => v << poly_subpix.SHIFT;
    ...
    private void AddVertex(VertexCmd cmd, double x, double y)
    {
        switch (cmd)
        {
    ...
    private bool _extendWidthX3ForSubPixelLcdEffect;
    ...
    //--------------------------------------------------------------------
    private int CalculateAlpha(int area)
    {
